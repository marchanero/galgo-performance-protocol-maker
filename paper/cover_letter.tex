...
We are pleased to submit our manuscript entitled \textbf{``A Comparative Study of Emerging Architectures for EDA-based Arousal Classification''} for consideration for publication in \textit{Biomedical Signal Processing and Control}.
...
The key finding is that Mamba, a selective state space model, achieves F1 = 0.858 at 1.5\,ms inference time, approaching PatchTST accuracy (F1 = 0.863) with 3.6$\times$ lower latency and 66\% fewer parameters. This difference does not survive Bonferroni-Holm correction for 28 comparisons, indicating that Mamba offers near-equivalent accuracy at substantially lower computational cost. These results directly inform architecture selection under deployment constraints in wearable, edge, and server scenarios.
