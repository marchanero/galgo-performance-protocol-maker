...
\noindent \textbf{Methods:} Four passive streams are recorded in synchrony: (i)~a custom ambient platform (temperature, humidity, CO\textsubscript{2}, PM2.5, PM10, illuminance, sound pressure level at 15~Hz); (ii)~EmotiBit wristbands (photoplethysmography at 25~Hz, electrodermal activity at 15~Hz, skin temperature, and 9-axis inertial measurement at 100~Hz, six units rotating weekly); (iii)~a 32-channel semi-dry EEG system at 256~Hz on one participant per week; and (iv)~overhead 4K video processed exclusively for body-pose features, with raw footage deleted within 72~hours. The cognitive-performance label is derived from the Behavioural Observation of Students in Schools (BOSS) system: a transformer-like encoder on pose sequences detects changes in classroom dynamics, and the teacher assigns BOSS codes via an ad-hoc annotation application. External validation uses the PVT administered monthly and an end-of-session 0--10 rating. The technical contribution is a dual-plane architecture using MQTT QoS~2 and NTP timestamping, targeting sub-100~ms inter-modality alignment. The study is observational; no intervention is delivered.

\noindent \textbf{Results:} The protocol was approved by the Ethics in Social Research Committee of the Universidad de Castilla-La Mancha (CEIS-UCLM, reference CEIS-738323-C7M2). The acquisition platform is implemented and bench-tested. The consent-and-assent campaign will open following approval; data collection is expected to begin in the subsequent school term and to run for four consecutive months. Results are anticipated within 12 months of deployment completion.
